...
interface CandidatosAdModalProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  candidatos: CandidatoPublic[]
}

export function CandidatosAdModal({ open, onOpenChange, candidatos }: CandidatosAdModalProps) {
  ...
  const getGridCols = () => {
    if (candidatos.length === 1) return 'grid-cols-1'
    if (candidatos.length === 2) return 'grid-cols-1 md:grid-cols-2'
  ...
  return (
    <Dialog open={open && candidatos.length > 0} onOpenChange={handleOpenChange}>
      <DialogContent
        className={cn(
      ...
      >
        <DialogTitle className="sr-only">Candidatos</DialogTitle>
        <motion.div
          key="modal-content"
        ...
        >
          <div className="text-center py-6 px-8 flex-shrink-0 border-b">
                <h2 className="text-3xl md:text-4xl font-bold">Candidatos</h2>
